...
export const RELATIONSHIP_CONTEXT = {
  description:
    'This temporal join adds the FIRST join activity (<%= appendActivityName %>) to the first primary activity (<%= cohortActivityName %>).',
  link: 'https://docs.narrator.ai/docs/relationships#first-ever',
  common_uses: [{ key: 1, text: "Add first ad source to customers' first sign up" }],
}
...
export const RELATIONSHIP_ANIMATION_STEPS = [
  ...
  },
  {
    ...
    customerJourney: {
      show: true,
      relationshipTitles: [{ title: 'First **Ever**', start: null, end: null }],
      customer: [
  ...
  {
    title: '"First Ever" - Choose the FIRST <%= appendActivityName %> from those activities',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false, highlighted: true },
  ...
  {
    title: 'Add the <%= appendActivityName %> activity to the corresponding <%= cohortActivityName %> in your dataset',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
