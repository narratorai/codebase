import { IRelationshipConstants } from '../interfaces'

export const PRIMARY_CUSTOMER = 'Harry'

export const CUSTOMER_JOURNEY_COHORT_LABELS = {
  [PRIMARY_CUSTOMER]: ['2018-06-01', null, null, '2018-12-11', null, '2019-01-21', '2019-02-04'],
}

export const RELATIONSHIP_CONTEXT = {
  description:
    'This temporal join aggregates all the join activities (<%= appendActivityName %> activities) that happened AFTER the primary activity (<%= cohortActivityName %>). Then the aggregate value (SUM, COUNT, MAX, etc) is appended to the dataset.',
  link: 'https://docs.narrator.ai/docs/relationships#aggregate-after',

  common_uses: [
    {
      key: 1,
      text: 'Enrich your dataset with the COUNT of all Calls Submitted after a Started Subscription (How many calls does each subscription generate?)',
    },
    {
      key: 2,
      text: 'Enrich your dataset with the SUM of the revenue after an upgrade.',
    },
  ],
}

export const RELATIONSHIP_TABLE_ROWS = [
  {
    key: '1',
    timestamp: '2018-06-01',
    customer: PRIMARY_CUSTOMER,
  },
  {
    key: '2',
    timestamp: '2018-12-11',
    customer: PRIMARY_CUSTOMER,
  },
  {
    key: '3',
    timestamp: '2019-01-21',
    customer: PRIMARY_CUSTOMER,
  },
  {
    key: '4',
    timestamp: '2019-02-04',
    customer: PRIMARY_CUSTOMER,
  },

  {
    key: '5',
    timestamp: '2018-06-04',
    customer: 'Sara',
  },
  {
    key: '6',
    timestamp: '2019-01-23',
    customer: 'Sara',
  },

  {
    key: '7',
    timestamp: '2018-06-07',
    customer: 'Sam',
  },
  {
    key: '8',
    timestamp: '2018-06-08',
    customer: 'Greg',
  },
  {
    key: '9',
    timestamp: '2018-06-08',
    customer: 'TJ',
  },
]

export const RELATIONSHIP_ANIMATION_STEPS = [
  {
    title: 'Start with your primary activity (<%= cohortActivityName %>)',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
      ],
      append: [
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
      ],
    },
    customerJourney: {
      show: false,
      customer: [],
    },
  },
  {
    title: 'Focus on a single customer',
    table: {
      show: true,
      focusCustomer: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
      ],
      append: [
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
      ],
    },
    customerJourney: {
      show: false,
      relationshipTitles: [],
      customer: [],
    },
  },
  {
    title: 'Look at their customer journey',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [],
      customer: [
        { type: 'cohort', ignored: false, section: 1 },
        { type: 'append', ignored: false, section: 1 },
        { type: 'append', ignored: false, section: 1, bottomMargin: true },
        { type: 'cohort', ignored: false, section: 2 },
        { type: 'append', ignored: false, section: 2, bottomMargin: true },
        { type: 'cohort', ignored: false, section: 2, bottomMargin: true },
        { type: 'cohort', ignored: false, section: 3 },
      ],
    },
  },
  {
    title: 'For each primary activity (<%= cohortActivityName %>) in your dataset, find it in the Activity Stream',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [],
      customer: [
        { type: 'cohort', ignored: false, section: 1, highlighted: true },
        { type: 'append', ignored: false, section: 1 },
        { type: 'append', ignored: false, section: 1, bottomMargin: true },
        { type: 'cohort', ignored: false, section: 2 },
        { type: 'append', ignored: false, section: 2, bottomMargin: true },
        { type: 'cohort', ignored: false, section: 2, bottomMargin: true },
        { type: 'cohort', ignored: false, section: 3 },
      ],
    },
  },
  {
    title: '"After" - Find all <%= appendActivityName %> activities that happened AFTER the <%= cohortActivityName %> ',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [{ title: 'Aggregate **After**', start: 0, end: null }],
      customer: [
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'append', ignored: false, leftMargin: true },
        { type: 'append', ignored: false, leftMargin: true, bottomMargin: true },
        { type: 'cohort', ignored: false },
        { type: 'append', ignored: false, leftMargin: true, bottomMargin: true },
        { type: 'cohort', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title: '"Aggregate After" - AGGREGATE the data from those <%= appendActivityName %> activities',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [{ title: '**Aggregate** After', start: 0, end: null, aggFunction: true }],
      customer: [
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'append', ignored: false, leftMargin: true, highlighted: true },
        { type: 'append', ignored: false, leftMargin: true, highlighted: true, bottomMargin: true },
        { type: 'cohort', ignored: false },
        { type: 'append', ignored: false, leftMargin: true, highlighted: true, bottomMargin: true },
        { type: 'cohort', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title:
      'Add the aggregated <%= appendActivityName %> data to the corresponding <%= cohortActivityName %> in your dataset',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title: 'Repeat: Focus on the next <%= cohortActivityName %> and find it in the Activity Stream',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title: '"After" - Find all <%= appendActivityName %> activities that happened AFTER the <%= cohortActivityName %> ',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [{ title: 'Aggregate **After**', start: 3, end: null }],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'append', ignored: false, leftMargin: true, bottomMargin: true },
        { type: 'cohort', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title: '"Aggregate After" - AGGREGATE the data from those <%= appendActivityName %> activities',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [{ title: '**Aggregate** After', start: 3, end: null, aggFunction: true }],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'append', ignored: false, leftMargin: true, highlighted: true, bottomMargin: true },
        { type: 'cohort', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title:
      'Add the aggregated <%= appendActivityName %> data to the corresponding <%= cohortActivityName %> in your dataset',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title: 'Repeat: Focus on the next <%= cohortActivityName %> and find it in the Activity Stream',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false, highlighted: true, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title: '"After" - Find all <%= appendActivityName %> activities that happened AFTER the <%= cohortActivityName %> ',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [{ title: 'Aggregate **After**', start: 5, end: null }],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false, highlighted: true, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title:
      'Add the aggregated <%= appendActivityName %> data to the corresponding <%= cohortActivityName %> in your dataset',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'value', value: 0, highlighted: true },
        { type: 'unknown', ignored: false },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [{ title: '**Aggregate** After', start: 5, end: null, aggFunction: true }],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: false, highlighted: true, bottomMargin: true },
        { type: 'cohort', ignored: false },
      ],
    },
  },
  {
    title: 'Repeat: Focus on the next <%= cohortActivityName %> and find it in the Activity Stream',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'value', value: 0 },
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true, bottomMargin: true },
        { type: 'cohort', ignored: false, highlighted: true },
      ],
    },
  },
  {
    title: '"After" - Find all <%= appendActivityName %> activities that happened AFTER the <%= cohortActivityName %> ',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'value', value: 0 },
        { type: 'unknown', ignored: false, highlighted: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [{ title: 'Aggregate **After**', start: 6, end: null }],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true, bottomMargin: true },
        { type: 'cohort', ignored: false, highlighted: true },
      ],
    },
  },
  {
    title: '"Aggregate After" - AGGREGATE the data from those <%= appendActivityName %> activities',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false, highlighted: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
        { type: 'cohort', ignored: true },
      ],
      append: [
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'value', value: 0 },
        { type: 'value', value: 0 },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
        { type: 'unknown', ignored: true },
      ],
    },
    customerJourney: {
      show: true,
      relationshipTitles: [{ title: '**Aggregate** After', start: 6, end: null, aggFunction: true }],
      customer: [
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true },
        { type: 'append', ignored: false, bottomMargin: true },
        { type: 'cohort', ignored: true, bottomMargin: true },
        { type: 'cohort', ignored: false, highlighted: true },
      ],
    },
  },
  {
    title: 'Repeat for all customers',
    table: {
      show: true,
      cohort: [
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
        { type: 'cohort', ignored: false },
      ],
      append: [
        { type: 'value', value: 0, ignored: false },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'value', value: 0 },
        { type: 'value', value: 0 },
        { type: 'value', value: 0, ignored: false },
        { type: 'append', ignored: false, aggFunction: true },
        { type: 'value', value: 0, ignored: false },
        { type: 'append', ignored: false, aggFunction: true },
      ],
    },
    customerJourney: {
      show: false,
      relationshipTitles: [],
      customer: [],
    },
  },
]

export default {
  PRIMARY_CUSTOMER,
  RELATIONSHIP_CONTEXT,
  RELATIONSHIP_TABLE_ROWS,
  RELATIONSHIP_ANIMATION_STEPS,
  CUSTOMER_JOURNEY_COHORT_LABELS,
} as IRelationshipConstants
