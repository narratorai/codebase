...
from batch_jobs.data_management.resync_narrative import resync_narrative
from core import utils
from core.api.customer_facing.reports.utils import NarrativeUpdator
from core.constants import GLAM_NARATIVE_V2
...
TITLE = "Create a Template V5"
DESCRIPTION = "Convert Narrative into a template, Updates templates, etc."
VERSION = 1
...
def get_schema(mavis: Mavis, internal_cache: dict):
    ...
    template_index = [t for t in graph_client.get_all_templates().narrative_template if t.global_version == 5]

    # get all the narratives
    all_templates = list({t.name for t in template_index})
    all_categories = list(set([t.category for t in template_index if t.category]))
    ...
    if is_loaded:
        ...
        if len(templates) > 0:
            valid_narratives = [
                dict(slug=n.slug, name=f"{n.name} ({n.slug})")
                for t in templates
        ...
        if not valid_narratives:
            valid_narratives = [
                dict(slug=n.slug, name=f"{n.name} ({n.slug})")
                for n in user_graph.narrative_index(company_id=mavis.company.id).narrative
            ]
    ...
    schema = _object(
        dict(
            template_name=(
                _drop_down(all_templates, title="Template Name") if all_templates else _input("Template Name")
            ),
            # add the actions
            delete=_checkbox("Delete"),
            delete_version=_checkbox("Delete Version"),
            save=_checkbox("save"),
            # Context
            template_context=_input(),
            top_side_pannel=_object(
                dict(
                    release_to_customer=_checkbox("Merge to Production"),
                    template_used_by=_number("Used by", default=0),
                    in_free_tier=_checkbox("Available to Everyone"),
                    category=_drop_down(all_categories, title="Category"),
                    # author=_drop_down(all_users, title="Author", default=mavis.user.id),
            ...
            cascade_update=_drop_down(
                ...
                "label",
                is_multi=True,
                title="Cascade to all Narratives",
            ),
        ...
        **_make_ui(
        ...
        delete_version=_make_ui(
            ...
            options=dict(
                ...
                tiny=True,
                popconfirm=True,
                popconfirm_text="Are you sure you want to delete the last version of this template?",
                **_space(10, mb=20, inline_button=True),
            ),
        ),
        save=_make_ui(
            ...
            options=dict(
                process_data=True,
                button_type="primary",
                **_space(20, mb=20, align_right=True),
            ),
        ),
        # add the details
        ...
        top_side_pannel=dict(
            **_make_ui(options=dict(title=False, **_space(30))),
            ...
            category=_make_ui(options=dict(allows_new_items=True)),
        ...
        # add All the context for the customer to understand what is happening
...
def _update_questions_with_previous_narrative_answers(questions, narrative):
    ...
    for q in questions:
        if answered_questions.get(q["id"]):
            ...
            if q["kind"] == "word" and not (q.get("word") or q["left"].get("word")):
...
def _get_narrative(mavis, nar_slug):
    ...
    nar_obj = user_graph.get_narrative_by_slug(slug=nar_slug, company_id=mavis.company.id).narrative[0]
...
def _question_to_template_mapper(questions):
    ...
    for q in questions:
        ...
        if q["kind"] == "word":
            ...
            # This is when we are loading the questions to ensure the feature that was used is being mapped to the correct question
            feature_mapping[q["new_feature_id"]] = q["id"]

        elif q["kind"] == "activity" and q.get("activity_id"):
...
def _remove_questions_using_unused_activities(questions, template):
    # remove any questions that are no longer needed
    ...
    questions = [
        ...
        if (
            ...
            or (q["kind"] == "activity" and q.get("activity_id") in all_activity_ids)
...
def create_narrative_template(mavis: Mavis, data, activities):
    # load the narrative
    (narrative, nar_obj) = _get_narrative(mavis, data["narrative_slug"])
    ...
    ignore_activity_and_feature = data["template_name"] == GLAM_NARATIVE_V2
    ...
    template = narrativeTemplate.create_template(
        ...
        activity_mapping,
        feature_mapping,
        ignore_activities_and_features=ignore_activity_and_feature,
    )

    # add the original link
    template.original_company_id = mavis.company.id
    template.original_narrative_slug = nar_obj.slug

    # Add the type to the narrative
    template.type = nar_obj.type
...
def _create_activity_question(mavis, am, dataset_objs, activity_objs):
    ...
    mapped_keys = dict(
        ...
        mix_name="Name with mixed cases",
...
def _update_word_question(q):
    ...
    basic_replace = set([sw.old_id for sw in wm])

    inflection_replace = set(
        [
            w.old_id
...
def create_questions(mavis, data, activities):
    ...
    for am in template.activity_mapping:
        ...
        if added_activities.get(am.old_id):
            ...
            # added_activities[am.old_id]["left"][
            #     "requires_revenue"
            # ] = am.requires_revenue
    ...
    # add the questions needed
    data["questions"] = questions

    # save the questions without their inputs
    ...
    if updated_field_slug and updated_field_slug.endswith("field_input") and data.get("narrative_slug"):
        ...
        if data["template_name"]:
            ...
            if updated_field_slug == "root_narrative_slug":
                if len(templates) == 0:
                    values = [
                        dict(value=n.slug, label=f"[{n.type}] {n.name}")
...
def process_data(mavis: Mavis, data: dict, updated_field_slug: str | None = None):
    if updated_field_slug == "root_right_context_load_preview":
        ...
        if data["cascade_all"]:
            ...
            data["cascade_update"] = [v["value"] for v in values]
        else:
            data["cascade_update"] = []
    ...
    elif updated_field_slug == "root_delete_version":
        # Delete the template
        if data["template_name"]:
            templates = graph_client.get_template_by_name(name=data["template_name"]).narrative_template

            graph_client.delete_template(id=templates[0].id)
        ...
        templates = [
            t
            for t in graph_client.get_template_by_name(name=data["template_name"]).narrative_template
    ...
    elif updated_field_slug == "root_narrative_slug":
        nar_obj = (
            make_sync_client(mavis.user.token)
            .get_narrative_by_slug(slug=data["narrative_slug"], company_id=mavis.company.id)
            .narrative[0]
        )

        nar_type = nar_obj.type.value

        data["template_context"] = [
            "Template original Narrative has been changed...",
            f"## {nar_type}  Used\n\n - {InternalLink(mavis.company.slug).narrative(nar_obj.slug, type=nar_type)} \n\n<br>",
        ]

    # check if the narrative slug is real
...
def _get_template_details(mavis: Mavis, templates, data, idx=0):
    context = []
    internal_link = InternalLink(mavis.company.slug)
    if len(templates) > 0:
        # utils.cprint(idx)
        lt = templates[idx]
        created_by = graph_client.get_user(id=lt.created_by or mavis.user.id).user_by_pk
        context.extend(
            [
        ...
        data["top_side_pannel"]["category"] = lt.category
        ...
        # utils.cprint(template_obj.template)
        ...
        # check if the template has narrative and if it is the same
        if template.original_narrative_slug:
            if template.original_company_id != mavis.company.id:
                template_company_slug = (
                    make_sync_client(mavis.user.token).get_company_slug(id=template.original_company_id).company_by_pk
                )

                if template_company_slug:
                    context.extend(
                        [
                            "## Template was created using a different company",
                            f" - Company Template: {PORTAL_URL}/{template_company_slug.slug}/manage/dynamic/create_narrative_template_v5",
                            f" - {template_obj.type.value.title()} Used: {internal_link.narrative(template.original_narrative_slug, override_company_slug = template_company_slug.slug, type=template_obj.type.value)}",
                        ]
                    )

                else:
                    context.append(
                        "This template was created by a different company that you don't have access to. Please contact support for help"
                    )
            else:
                # use the Narrative from the template instead
                data["narrative_slug"] = template.original_narrative_slug
                nar_obj = graph_client.get_narrative_by_slug(
                    slug=data["narrative_slug"], company_id=mavis.company.id
                ).narrative[0]
                if not template_obj.type:
                    template_obj.type = nar_obj.type

                # handling Archived Narratives
                if nar_obj.state == nar_obj.state.archived:
                    context.insert(
                        0,
                        "`⚠️ This template was created using an archived Narrative. Please unarchive the Narrative to use this template`",
                    )

                context.insert(
                    0,
                    f"## {template_obj.type.value.title()}  Used\n\n - {internal_link.narrative(template.original_narrative_slug, type=template_obj.type.value)} \n\n<br>",
                )

                found = True
        elif len(lt.narratives) > 0 and lt.narratives[0].company.id != mavis.company.id:
            context.extend(
                [
                    utils.bad_text("This company doesn't have the most recent changes of this template \n\n <br>"),
                    f"Please switch companies to **{lt.narratives[0].company.name}**: {PORTAL_URL}/{lt.narratives[0].company.slug}/manage/dynamic/create_narrative_template_v5",
                ]
            )

        # add all historical
        context.append("<details><summary>Past Production Narratives Used </summary>")
            ...
            if ii == 0:
                for n in t.narratives:
                    ...
                    if n.company.id == mavis.company.id and not found:
                        ...
                        if not template_obj.type:
                            template_obj.type = n.type

                        context.insert(
                            0,
                            f"## {template_obj.type.value} Used: [{n.name}]({internal_link.narrative(n.slug, type=template_obj.type.value)}) \n\n<br>",
                        )
                        found = True

                    context.append(
                        f" 1. V{t.customer_iteration}.{t.local_iteration} [{n.name}]({internal_link.narrative(n.slug, override_company_slug=n.company.slug, type=template_obj.type.value)} - {n.company.name} *updated {utils.pretty_diff(n.updated_at)}*"
                    )
...
def _save_template(mavis, template, data, activities):
    # update all references (swap old_id with the new_id)
    narrativeTemplate.prepare_template(template, additional_context=data["additional_context"])
    ...
    template.ignore_activities_and_features = data["template_name"] == GLAM_NARATIVE_V2
    ...
    template_id = graph_client.insert_narrative_template(
        ...
        display_company_using=data["top_side_pannel"]["template_used_by"],
        category=data["top_side_pannel"]["category"] or None,
        # if not created by then just make it Ahmed (used for local testing)
        created_by=mavis.user.id,
        template=utils.compact_json_dumps(utils.remove_enums(template.dict())),
        company_id=None,
        type=template.type.value or "analysis",
    ...
    # Delete all the old templates
    if (new_customer_i - 2) >= 0:
        graph_client.delete_template_by_name(
            name=data["template_name"],
            max_customer_iteration=new_customer_i - 2,
        )


def _get_cascade_template(mavis, templates, narrative_slug):
    values = []
    # get all the template data
    for t in templates:
        for nar in t.narratives:
