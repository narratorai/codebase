...
class BaseElasticSearch(BaseQueryRunner):
    ...
    def get_schema(self, *args, **kwargs):
        ...
        if mappings:
            ...
            for name, index in mappings.items():
                ...
                for _, items in index["mappings"].items():
    ...
    def test_connection(self):
        try:
            r = requests.get(f"{self.server_url}/_cluster/health", auth=self.auth)  # noqa: ASYNC100
            r.raise_for_status()
        except requests.HTTPError as e:
            logger.exception(e)
...
class ElasticSearch(BaseElasticSearch):
    ...
    def run_query(self, query, user):
        ...
        try:
            ...
            r = requests.get(url, json=query_dict, auth=self.auth)  # noqa: ASYNC100
            r.raise_for_status()
