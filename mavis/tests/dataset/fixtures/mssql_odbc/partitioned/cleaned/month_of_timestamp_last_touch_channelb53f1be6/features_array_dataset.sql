...
cohort_stream AS (
	...
	WHERE s.ts >= CAST('2022-01-01T05:00:00.000' AS DATETIME)
),
before_stream AS (
	SELECT
		*
...
SELECT
	*
	, total_started_pro_subscription_rows / NULLIF(SUM(total_started_pro_subscription_rows) OVER (PARTITION BY month_of_timestamp), 0) AS "percent_of_started_session"
FROM (
	SELECT
