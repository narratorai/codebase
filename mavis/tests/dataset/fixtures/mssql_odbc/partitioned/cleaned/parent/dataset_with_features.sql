...
append_ever AS (
	SELECT
		...
		, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'company_started_subscription' THEN CONCAT(LEFT(s.ts, 19), COALESCE(s.link,'')) END ),20, 1000),'') AS "first_ever_company_started_subscriptions_link"
