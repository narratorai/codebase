...
append_ever AS (
	SELECT
		...
		, COUNT(CASE WHEN ( s.activity = 'transaction' AND CAST(JSON_VALUE(s.feature_json, '$.a2') AS FLOAT) > 0.0 ) THEN s.ts END) AS "total_transactions_ever"
		, SUM(CASE WHEN ( s.activity = 'transaction' AND CAST(JSON_VALUE(s.feature_json, '$.a2') AS FLOAT) > 0.0 ) THEN CAST(JSON_VALUE(s.feature_json, '$.a2') AS FLOAT) END) AS "sum_of_transaction_volume_usd_ever"
		, MAX(CASE WHEN ( s.activity = 'transaction' AND CAST(JSON_VALUE(s.feature_json, '$.a2') AS FLOAT) > 0.0 ) THEN s.ts END) AS "last_ever_transactions_timestamp"
		, MAX(CASE WHEN s.activity = 'marketplace_signup' THEN s.ts END) AS "last_ever_marketplace_signups_timestamp"
		, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'marketplace_signup' THEN CONCAT(LEFT(s.ts, 19), COALESCE(JSON_VALUE(s.feature_json, '$.a1'),'')) END ),20, 1000),'') AS "last_ever_how_did_you_hear_about_us"
		...
		, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'deal_created' THEN CONCAT(LEFT(s.ts, 19), COALESCE(JSON_VALUE(s.feature_json, '$.a3'),'')) END ),20, 1000),'') AS "last_ever_deal_channel"
		, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'deal_created' THEN CONCAT(LEFT(s.ts, 19), COALESCE(s.link,'')) END ),20, 1000),'') AS "last_ever_deal_createds_link"
...
FROM (
	SELECT
		...
		, FLOOR(DATEDIFF(second, join_ts, last_ever_calls_timestamp)/86400) AS "days_to_call"
		, CASE WHEN last_ever_voicemails_timestamp is not NULL THEN 1 ELSE 0 END AS "did_voicemail_ever"
		, FLOOR(DATEDIFF(second, join_ts, last_ever_voicemails_timestamp)/86400) AS "days_to_voicemail"
		, CASE WHEN last_ever_opened_emails_timestamp is not NULL THEN 1 ELSE 0 END AS "did_opened_email_ever"
		, FLOOR(DATEDIFF(second, join_ts, last_ever_opened_emails_timestamp)/86400) AS "days_to_opened_email"
		, CASE WHEN last_ever_clicked_docsends_timestamp is not NULL THEN 1 ELSE 0 END AS "did_clicked_docsend_ever"
		, FLOOR(DATEDIFF(second, join_ts, last_ever_clicked_docsends_timestamp)/86400) AS "days_to_clicked_docsend"
		, CASE WHEN last_ever_transactions_timestamp is not NULL THEN 1 ELSE 0 END AS "did_transaction_ever"
		, FLOOR(DATEDIFF(second, join_ts, last_ever_transactions_timestamp)/86400) AS "days_to_transaction"
		, CASE WHEN last_ever_marketplace_signups_timestamp is not NULL THEN 1 ELSE 0 END AS "did_marketplace_signup_ever"
		, FLOOR(DATEDIFF(second, join_ts, last_ever_marketplace_signups_timestamp)/86400) AS "days_to_marketplace_signup"
		, CASE WHEN last_ever_deal_createds_timestamp is not NULL THEN 1 ELSE 0 END AS "did_deal_created_ever"
		, FLOOR(DATEDIFF(second, join_ts, last_ever_deal_createds_timestamp)/86400) AS "days_to_deal_created"
		, CASE WHEN first_ever_lead_funnel_entereds_timestamp is not NULL THEN 1 ELSE 0 END AS "did_lead_funnel_entered_ever"
		, FLOOR(DATEDIFF(second, first_ever_lead_funnel_entereds_timestamp, join_ts)/86400) AS "days_from_lead_funnel_entered"
		...
		, FLOOR(DATEDIFF(second, first_ever_subscribed_to_newsletters_timestamp, join_ts)/86400) AS "days_from_subscribed_to_newsletter"
		, CASE WHEN last_ever_received_emails_timestamp is not NULL THEN 1 ELSE 0 END AS "did_received_email_ever"
		, FLOOR(DATEDIFF(second, join_ts, last_ever_received_emails_timestamp)/86400) AS "days_to_received_email"
		, CASE WHEN last_ever_sent_emails_timestamp is not NULL THEN 1 ELSE 0 END AS "did_sent_email_ever"
		, FLOOR(DATEDIFF(second, join_ts, last_ever_sent_emails_timestamp)/86400) AS "days_to_sent_email"
		, CASE WHEN last_ever_sales_meetings_timestamp is not NULL THEN 1 ELSE 0 END AS "did_sales_meeting_ever"
		, FLOOR(DATEDIFF(second, join_ts, last_ever_sales_meetings_timestamp)/86400) AS "days_to_sales_meeting"
		, DATEADD(month, datediff(month, 0, timestamp), 0) AS "month"
		, FLOOR(DATEDIFF(second, last_ever_transactions_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_last_transaction"
		, FLOOR(DATEDIFF(second, last_ever_clicked_docsends_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_last_docsend"
		, FLOOR(DATEDIFF(second, last_ever_opened_emails_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_last_email_open"
		, FLOOR(DATEDIFF(second, last_ever_received_emails_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_last_received_email"
		, FLOOR(DATEDIFF(second, last_ever_sent_emails_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_last_sent_email"
		, FLOOR(DATEDIFF(second, last_ever_voicemails_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_last_voicemail"
		, FLOOR(DATEDIFF(second, last_ever_calls_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_last_call"
		, FLOOR(DATEDIFF(second, timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_added_to_crm"
		, FLOOR(DATEDIFF(second, last_ever_marketplace_signups_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_mp_signup"
		, FLOOR(DATEDIFF(second, last_ever_deal_createds_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_last_product_deal_created"
		, FLOOR(DATEDIFF(second, last_ever_sales_meetings_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_last_meeting"
		, FLOOR(DATEDIFF(second, first_ever_lead_funnel_entereds_timestamp, CURRENT_TIMESTAMP)/86400) AS "days_since_lead_funnel_entered"
	FROM cohort AS c
