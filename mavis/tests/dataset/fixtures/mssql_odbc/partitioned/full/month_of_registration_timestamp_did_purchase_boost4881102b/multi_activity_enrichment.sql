...
append_after AS (
	...
	FROM (
		SELECT
			...
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'purchased_premium' THEN CONCAT(LEFT(s.ts, 19), COALESCE(JSON_VALUE(s.feature_json, '$.a2'),'')) END ),20, 1000),'') AS "first_purchase_is_trial_conversion"
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'purchased_premium' THEN CONCAT(LEFT(s.ts, 19), COALESCE(s.activity_id,'')) END ),20, 1000),'') AS "join_enriched_activity_id_1_a"
			, MIN(CASE WHEN s.activity = 'bought_ppp' THEN s.ts END) AS "first_ppp_bought_timestamp"
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'bought_ppp' THEN CONCAT(LEFT(s.ts, 19), COALESCE(JSON_VALUE(s.feature_json, '$.a1'),'')) END ),20, 1000),'') AS "first_ppp_feature"
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'bought_ppp' THEN CONCAT(LEFT(s.ts, 19), COALESCE(JSON_VALUE(s.feature_json, '$.a2'),'')) END ),20, 1000),'') AS "first_ppp_location"
...
raw_dataset AS (
	...
	FROM (
		SELECT
			...
			, DATEADD(month, datediff(month, 0, registration_timestamp), 0) AS "month_of_registration_timestamp"
		FROM cohort AS c
