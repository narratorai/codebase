...
append_before AS (
	...
	FROM (
		SELECT
			...
			, NULLIF(SUBSTRING(MAX(CASE WHEN ( s.activity = 'started_marketing_session' AND ABS(FLOOR(DATEDIFF(second, s.ts, c.join_ts)/86400)) < 30 ) THEN CONCAT(LEFT(s.ts, 19), COALESCE(s.activity_id,'')) END ),20, 1000),'') AS "join_enriched_activity_id_0_a"
		FROM cohort AS c
...
append_ever AS (
	SELECT
		...
		, NULLIF(SUBSTRING(MIN(CASE WHEN ( s.activity = 'company_started_subscription' AND s.link is not NULL ) THEN CONCAT(LEFT(s.ts, 19), COALESCE(s.link,'')) END ),20, 1000),'') AS "first_ever_company_started_subscriptions_link"
...
append_in_between AS (
	SELECT
		...
		, NULLIF(SUBSTRING(MIN(CONCAT(LEFT(s.first_in_between_viewed_dashboards_timestamp, 19), COALESCE(first_in_between_company_slug,''))),20, 1000),'') AS "first_in_between_company_slug"
		, NULLIF(SUBSTRING(MIN(CONCAT(LEFT(s.first_in_between_viewed_dashboards_timestamp, 19), COALESCE(first_in_between_slug,''))),20, 1000),'') AS "first_in_between_slug"
	FROM (
		SELECT
			COALESCE(s.customer, s.anonymous_customer_id) AS "join_customer"
			, c.join_cohort_id
			...
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(FLOOR(DATEDIFF(second, c.join_ts, s.ts)/60)) < 30 ) THEN JSON_VALUE(s.feature_json, '$.company_slug') END AS "first_in_between_company_slug"
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(FLOOR(DATEDIFF(second, c.join_ts, s.ts)/60)) < 30 ) THEN JSON_VALUE(s.feature_json, '$.slug') END AS "first_in_between_slug"
		FROM cohort AS c
