...
cohort AS (
	...
	FROM (
		SELECT
			...
			, s.ts AS "timestamp"
			, s.customer
			, CAST(s.feature_json."a1" AS VARCHAR(4096)) AS "plan_type"
			, CASE WHEN CAST(s.feature_json."a2" AS VARCHAR(4096)) in (NULL, '') THEN NULL ELSE (LOWER(CAST(s.feature_json."a2" AS VARCHAR(4096))) = 'true') END AS "trial_plan"
	...
	FROM (
		...
		FROM (
			SELECT
				...
				, c.join_cohort_id
				, s.ts
				, CASE WHEN ( s.activity = 'started_session' AND ABS(FLOOR(DATE_DIFF('second', s.ts, c.join_ts)/86400)) < 30 ) THEN s.ts END AS "last_before_started_session_timestamp"
				, CASE WHEN ( s.activity = 'started_session' AND ABS(FLOOR(DATE_DIFF('second', s.ts, c.join_ts)/86400)) < 30 ) THEN s.activity_id END AS "join_enriched_activity_id_0_a"
				, CASE WHEN ( s.activity = 'clicked_email_link' AND ( CAST(s.feature_json."a2" AS VARCHAR(4096)) not ilike '%password reset%' OR CAST(s.feature_json."a2" AS VARCHAR(4096)) not ilike '%validation%' ) AND ABS(FLOOR(DATE_DIFF('second', s.ts, c.join_ts)/86400)) < 30 ) THEN s.ts END AS "last_before_email_link_clicked_timestamp"
				, CASE WHEN ( s.activity = 'clicked_email_link' AND ( CAST(s.feature_json."a2" AS VARCHAR(4096)) not ilike '%password reset%' OR CAST(s.feature_json."a2" AS VARCHAR(4096)) not ilike '%validation%' ) AND ABS(FLOOR(DATE_DIFF('second', s.ts, c.join_ts)/86400)) < 30 ) THEN s.link END AS "last_before_email_link_clicked_link"
				, CASE WHEN ( s.activity = 'clicked_email_link' AND ( CAST(s.feature_json."a2" AS VARCHAR(4096)) not ilike '%password reset%' OR CAST(s.feature_json."a2" AS VARCHAR(4096)) not ilike '%validation%' ) AND ABS(FLOOR(DATE_DIFF('second', s.ts, c.join_ts)/86400)) < 30 ) THEN CAST(s.feature_json."a2" AS VARCHAR(4096)) END AS "last_before_campaign_name"
				, CASE WHEN s.activity = 'payment' THEN s.ts END AS "last_before_payment_transactions_timestamp"
				, CASE WHEN s.activity = 'payment' THEN CAST(s.feature_json."a3" AS VARCHAR(4096)) END AS "last_before_pro_type"
...
raw_dataset AS (
	...
	FROM (
		SELECT
			...
			, FLOOR(DATE_DIFF('second', last_before_started_session_timestamp, join_ts)/86400) AS "days_from_started_session"
			, CASE WHEN last_before_email_link_clicked_timestamp is not NULL THEN 1 ELSE 0 END AS "did_email_link_clicked_before"
			, FLOOR(DATE_DIFF('second', last_before_email_link_clicked_timestamp, join_ts)/86400) AS "days_from_email_link_clicked"
			, CASE WHEN last_before_payment_transactions_timestamp is not NULL THEN 1 ELSE 0 END AS "did_payment_transactions_before"
			, FLOOR(DATE_DIFF('second', last_before_payment_transactions_timestamp, join_ts)/86400) AS "days_from_payment_transactions"
...
SELECT
	*
	, RATIO_TO_REPORT(total_started_pro_subscription_rows) OVER () AS "percent_of_email_link_clicked"
FROM (
	SELECT
		rd.last_before_email_link_clicked_link AS "last_before_email_link_clicked_link"
	...
	GROUP BY rd.last_before_email_link_clicked_link
)
