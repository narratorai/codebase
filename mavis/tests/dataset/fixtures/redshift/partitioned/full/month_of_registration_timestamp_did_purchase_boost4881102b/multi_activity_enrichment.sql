...
append_after AS (
	...
	FROM (
		SELECT
			...
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'purchased_premium' THEN CONCAT(LEFT(s.ts, 19), NVL(CAST(s.feature_json."a2" AS VARCHAR(4096)),'')) END ),20, 1000),'') AS "first_purchase_is_trial_conversion"
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'purchased_premium' THEN CONCAT(LEFT(s.ts, 19), NVL(s.activity_id,'')) END ),20, 1000),'') AS "join_enriched_activity_id_1_a"
			, MIN(CASE WHEN s.activity = 'bought_ppp' THEN s.ts END) AS "first_ppp_bought_timestamp"
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'bought_ppp' THEN CONCAT(LEFT(s.ts, 19), NVL(CAST(s.feature_json."a1" AS VARCHAR(4096)),'')) END ),20, 1000),'') AS "first_ppp_feature"
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'bought_ppp' THEN CONCAT(LEFT(s.ts, 19), NVL(CAST(s.feature_json."a2" AS VARCHAR(4096)),'')) END ),20, 1000),'') AS "first_ppp_location"
			, COUNT(CASE WHEN ( s.activity in ('purchased_premium', 'made_purchase') AND ABS(FLOOR(DATE_DIFF('second', c.join_ts, s.ts)/86400)) < 100 AND ( enriched_purchase_tbl.is_change_during_sub_period <> True OR enriched_purchase_tbl.is_change_during_sub_period is NULL ) ) THEN s.ts END) AS "total_purchases"
			, SUM(CASE WHEN ( s.activity in ('purchased_premium', 'made_purchase') AND ABS(FLOOR(DATE_DIFF('second', c.join_ts, s.ts)/86400)) < 100 AND ( enriched_purchase_tbl.is_change_during_sub_period <> True OR enriched_purchase_tbl.is_change_during_sub_period is NULL ) ) THEN s.revenue_impact END) AS "a_3_mo_ltv"
...
raw_dataset AS (
	...
	FROM (
		SELECT
			...
			, FLOOR(DATE_DIFF('second', join_ts, first_trial_timestamp)/86400) AS "days_to_first_redeem_trial"
			, CASE WHEN first_premium_purchase_timestamp is not NULL THEN 1 ELSE 0 END AS "did_purchased_premium"
			, FLOOR(DATE_DIFF('second', join_ts, first_premium_purchase_timestamp)/86400) AS "days_to_first_purchased_premium"
			, CASE WHEN first_ppp_bought_timestamp is not NULL THEN 1 ELSE 0 END AS "did_trigger_bought_ppp"
			, FLOOR(DATE_DIFF('second', join_ts, first_ppp_bought_timestamp)/86400) AS "days_to_first_bought_ppp"
			, CASE WHEN first_boost_purchase_timestamp is not NULL THEN 1 ELSE 0 END AS "did_purchase_boost"
			, FLOOR(DATE_DIFF('second', join_ts, first_boost_purchase_timestamp)/86400) AS "days_to_first_boost_purchase"
