...
cohort AS (
	SELECT
		...
		, CAST(s.feature_json."did_order" AS BOOL) AS "did_order"
		, s.feature_json."ingredient_id" AS "ingredient_id"
		, CAST(s.feature_json."is_swapped" AS BOOL) AS "is_swapped"
	...
	FROM (
		...
		FROM (
			SELECT
				...
				, c.join_cohort_id
				, s.ts
				, CASE WHEN ( s.activity = 'considered_ordering_ingredient' AND CAST(s.feature_json."did_order" AS BOOL) = True AND s.feature_json."stock_ingredient_id" = c.app_feature_stock_ingredient_id ) THEN s.ts END AS "last_before_considered_ordering_ingredients_timestamp"
				, CASE WHEN ( s.activity = 'considered_ordering_ingredient' AND CAST(s.feature_json."did_order" AS BOOL) = True AND s.feature_json."stock_ingredient_id" = c.app_feature_stock_ingredient_id ) THEN s.activity_id END AS "last_before_considered_ordering_ingredients_activity_id"
				, CASE WHEN ( s.activity = 'considered_ordering_ingredient' AND CAST(s.feature_json."did_order" AS BOOL) = True AND s.feature_json."stock_ingredient_id" = c.app_feature_stock_ingredient_id AND ROW_NUMBER() over (PARTITION by s.activity, s.customer, c.join_cohort_id, CASE WHEN ( CAST(s.feature_json."did_order" AS BOOL) = True AND s.feature_json."stock_ingredient_id" = c.app_feature_stock_ingredient_id ) THEN 1 END ORDER BY s.ts desc) = 1 ) THEN s.feature_json."product_requirement" END AS "last_before_product_requirement_activity"
				, CASE WHEN ( s.activity = 'considered_ordering_ingredient' AND CAST(s.feature_json."did_order" AS BOOL) = True AND s.feature_json."stock_ingredient_id" = c.app_feature_stock_ingredient_id ) THEN CAST(s.feature_json."ingredient_id" AS VARCHAR(4096)) END AS "join_enriched_activity_id_0_a"
			FROM cohort AS c
			INNER JOIN before_stream AS s
				ON (
					s.customer = c.join_customer  AND
					(
						CAST(s.feature_json."did_order" AS BOOL) = True  AND
...
FROM (
	SELECT
		...
		, FLOOR(DATE_DIFF('second', last_before_considered_ordering_ingredients_timestamp, join_ts)/60) AS "minutes_from_considered_ordering_ingredient"
	FROM cohort AS c
