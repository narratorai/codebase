...
append_ever AS (
	SELECT
		...
		, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'company_started_subscription' THEN CONCAT(LEFT(s.ts, 19), NVL(s.link,'')) END ),20, 1000),'') AS "first_ever_company_started_subscriptions_link"
...
append_in_between AS (
	...
	FROM (
		SELECT
			...
			, CASE WHEN s.activity = 'viewed_dashboard' THEN CAST(s.feature_json."company_slug" AS VARCHAR(4096)) END AS "first_in_between_company_slug"
			, CASE WHEN s.activity = 'viewed_dashboard' THEN CAST(s.feature_json."slug" AS VARCHAR(4096)) END AS "first_in_between_slug"
		FROM cohort AS c
