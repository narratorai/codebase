...
append_before AS (
	...
	FROM (
		SELECT
			NVL(s.customer, s.anonymous_customer_id) AS "join_customer"
			, c.join_cohort_id
			, MAX(CASE WHEN ( s.activity = 'started_marketing_session' AND ABS(FLOOR(DATE_DIFF('second', s.ts, c.join_ts)/86400)) < 30 ) THEN s.ts END) AS "last_before_started_marketing_sessions_timestamp"
			, NULLIF(SUBSTRING(MAX(CASE WHEN ( s.activity = 'started_marketing_session' AND ABS(FLOOR(DATE_DIFF('second', s.ts, c.join_ts)/86400)) < 30 ) THEN CONCAT(LEFT(s.ts, 19), NVL(s.activity_id,'')) END ),20, 1000),'') AS "join_enriched_activity_id_0_a"
		FROM cohort AS c
...
append_ever AS (
	SELECT
		...
		, NULLIF(SUBSTRING(MIN(CASE WHEN ( s.activity = 'company_started_subscription' AND s.link is not NULL ) THEN CONCAT(LEFT(s.ts, 19), NVL(s.link,'')) END ),20, 1000),'') AS "first_ever_company_started_subscriptions_link"
...
append_in_between AS (
	SELECT
		...
		, NULLIF(SUBSTRING(MIN(CONCAT(LEFT(s.first_in_between_viewed_dashboards_timestamp, 19), NVL(first_in_between_company_slug,''))),20, 1000),'') AS "first_in_between_company_slug"
		, NULLIF(SUBSTRING(MIN(CONCAT(LEFT(s.first_in_between_viewed_dashboards_timestamp, 19), NVL(first_in_between_slug,''))),20, 1000),'') AS "first_in_between_slug"
	FROM (
		SELECT
			NVL(s.customer, s.anonymous_customer_id) AS "join_customer"
			, c.join_cohort_id
			, s.ts
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(FLOOR(DATE_DIFF('second', c.join_ts, s.ts)/60)) < 30 ) THEN s.ts END AS "first_in_between_viewed_dashboards_timestamp"
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(FLOOR(DATE_DIFF('second', c.join_ts, s.ts)/60)) < 30 ) THEN CAST(s.feature_json."company_slug" AS VARCHAR(4096)) END AS "first_in_between_company_slug"
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(FLOOR(DATE_DIFF('second', c.join_ts, s.ts)/60)) < 30 ) THEN CAST(s.feature_json."slug" AS VARCHAR(4096)) END AS "first_in_between_slug"
		FROM cohort AS c
		INNER JOIN in_between_stream AS s
			ON (
				NVL(s.customer, s.anonymous_customer_id) = c.join_customer  AND
				ABS(FLOOR(DATE_DIFF('second', c.join_ts, s.ts)/60)) < 30  AND
				s.ts > c.join_ts  AND
...
SELECT
	...
	, FLOOR(DATE_DIFF('second', last_before_started_marketing_sessions_timestamp, join_ts)/86400) AS "days_from_started_marketing_session"
	...
	, FLOOR(DATE_DIFF('second', first_ever_company_started_subscriptions_timestamp, join_ts)/86400) AS "days_from_company_started_subscription"
	, CASE WHEN first_in_between_viewed_dashboards_timestamp is not NULL THEN 1 ELSE 0 END AS "did_viewed_dashboard_between"
	, FLOOR(DATE_DIFF('second', join_ts, first_in_between_viewed_dashboards_timestamp)/86400) AS "days_to_viewed_dashboard"
FROM cohort AS c
