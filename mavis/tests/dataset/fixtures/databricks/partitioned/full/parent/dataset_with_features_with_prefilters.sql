...
append_before AS (
	...
	FROM (
		SELECT
			...
			, NULLIF(SUBSTRING(MAX(CASE WHEN ( s.activity = 'started_marketing_session' AND ABS(FLOOR(datediff(second, s.ts, c.join_ts)/86400)) < 30 ) THEN CONCAT(LEFT(s.ts, 19), NVL(s.activity_id,'')) END ),20, 1000),'') AS join_enriched_activity_id_0_a
		FROM cohort AS c
...
append_in_between AS (
	...
	FROM (
		SELECT
			...
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(FLOOR(datediff(second, c.join_ts, s.ts)/60)) < 30 ) THEN s.feature_json:company_slug END AS first_in_between_company_slug
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(FLOOR(datediff(second, c.join_ts, s.ts)/60)) < 30 ) THEN s.feature_json:slug END AS first_in_between_slug
		FROM cohort AS c
