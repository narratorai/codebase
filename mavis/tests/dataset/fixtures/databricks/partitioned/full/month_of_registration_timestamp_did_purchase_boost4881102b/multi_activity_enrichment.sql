...
append_after AS (
	...
	FROM (
		SELECT
			...
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'purchased_premium' THEN CONCAT(LEFT(s.ts, 19), NVL(s.feature_json:a2,'')) END ),20, 1000),'') AS first_purchase_is_trial_conversion
			...
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'bought_ppp' THEN CONCAT(LEFT(s.ts, 19), NVL(s.feature_json:a1,'')) END ),20, 1000),'') AS first_ppp_feature
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'bought_ppp' THEN CONCAT(LEFT(s.ts, 19), NVL(s.feature_json:a2,'')) END ),20, 1000),'') AS first_ppp_location
