...
append_ever AS (
	SELECT
		...
		, COUNT(CASE WHEN ( s.activity = 'transaction' AND FLOAT(s.feature_json:a2) > 0.0 ) THEN s.ts END) AS total_transactions_ever
		, SUM(CASE WHEN ( s.activity = 'transaction' AND FLOAT(s.feature_json:a2) > 0.0 ) THEN FLOAT(s.feature_json:a2) END) AS sum_of_transaction_volume_usd_ever
		, MAX(CASE WHEN ( s.activity = 'transaction' AND FLOAT(s.feature_json:a2) > 0.0 ) THEN s.ts END) AS last_ever_transactions_timestamp
		, MAX(CASE WHEN s.activity = 'marketplace_signup' THEN s.ts END) AS last_ever_marketplace_signups_timestamp
		, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'marketplace_signup' THEN CONCAT(LEFT(s.ts, 19), NVL(s.feature_json:a1,'')) END ),20, 1000),'') AS last_ever_how_did_you_hear_about_us
		...
		, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'deal_created' THEN CONCAT(LEFT(s.ts, 19), NVL(s.feature_json:a3,'')) END ),20, 1000),'') AS last_ever_deal_channel
...
FROM (
	SELECT
		...
		, FLOOR(datediff(second, last_ever_transactions_timestamp, NOW())/86400) AS days_since_last_transaction
		, FLOOR(datediff(second, last_ever_clicked_docsends_timestamp, NOW())/86400) AS days_since_last_docsend
		, FLOOR(datediff(second, last_ever_opened_emails_timestamp, NOW())/86400) AS days_since_last_email_open
		, FLOOR(datediff(second, last_ever_received_emails_timestamp, NOW())/86400) AS days_since_last_received_email
		, FLOOR(datediff(second, last_ever_sent_emails_timestamp, NOW())/86400) AS days_since_last_sent_email
		, FLOOR(datediff(second, last_ever_voicemails_timestamp, NOW())/86400) AS days_since_last_voicemail
		, FLOOR(datediff(second, last_ever_calls_timestamp, NOW())/86400) AS days_since_last_call
		, FLOOR(datediff(second, timestamp, NOW())/86400) AS days_since_added_to_crm
		, FLOOR(datediff(second, last_ever_marketplace_signups_timestamp, NOW())/86400) AS days_since_mp_signup
		, FLOOR(datediff(second, last_ever_deal_createds_timestamp, NOW())/86400) AS days_since_last_product_deal_created
		, FLOOR(datediff(second, last_ever_sales_meetings_timestamp, NOW())/86400) AS days_since_last_meeting
		, FLOOR(datediff(second, first_ever_lead_funnel_entereds_timestamp, NOW())/86400) AS days_since_lead_funnel_entered
	FROM cohort AS c
