...
FROM (
	SELECT
		...
		, floor(EXTRACT(EPOCH FROM (last_ever_calls_timestamp - join_ts)) / 86400) AS "days_to_call"
		, CASE WHEN last_ever_voicemails_timestamp is not NULL THEN 1 ELSE 0 END AS "did_voicemail_ever"
		, floor(EXTRACT(EPOCH FROM (last_ever_voicemails_timestamp - join_ts)) / 86400) AS "days_to_voicemail"
		, CASE WHEN last_ever_opened_emails_timestamp is not NULL THEN 1 ELSE 0 END AS "did_opened_email_ever"
		, floor(EXTRACT(EPOCH FROM (last_ever_opened_emails_timestamp - join_ts)) / 86400) AS "days_to_opened_email"
		, CASE WHEN last_ever_clicked_docsends_timestamp is not NULL THEN 1 ELSE 0 END AS "did_clicked_docsend_ever"
		, floor(EXTRACT(EPOCH FROM (last_ever_clicked_docsends_timestamp - join_ts)) / 86400) AS "days_to_clicked_docsend"
		, CASE WHEN last_ever_transactions_timestamp is not NULL THEN 1 ELSE 0 END AS "did_transaction_ever"
		, floor(EXTRACT(EPOCH FROM (last_ever_transactions_timestamp - join_ts)) / 86400) AS "days_to_transaction"
		, CASE WHEN last_ever_marketplace_signups_timestamp is not NULL THEN 1 ELSE 0 END AS "did_marketplace_signup_ever"
		, floor(EXTRACT(EPOCH FROM (last_ever_marketplace_signups_timestamp - join_ts)) / 86400) AS "days_to_marketplace_signup"
		, CASE WHEN last_ever_deal_createds_timestamp is not NULL THEN 1 ELSE 0 END AS "did_deal_created_ever"
		, floor(EXTRACT(EPOCH FROM (last_ever_deal_createds_timestamp - join_ts)) / 86400) AS "days_to_deal_created"
		, CASE WHEN first_ever_lead_funnel_entereds_timestamp is not NULL THEN 1 ELSE 0 END AS "did_lead_funnel_entered_ever"
		, floor(EXTRACT(EPOCH FROM (join_ts - first_ever_lead_funnel_entereds_timestamp)) / 86400) AS "days_from_lead_funnel_entered"
		...
		, floor(EXTRACT(EPOCH FROM (join_ts - first_ever_subscribed_to_newsletters_timestamp)) / 86400) AS "days_from_subscribed_to_newsletter"
		, CASE WHEN last_ever_received_emails_timestamp is not NULL THEN 1 ELSE 0 END AS "did_received_email_ever"
		, floor(EXTRACT(EPOCH FROM (last_ever_received_emails_timestamp - join_ts)) / 86400) AS "days_to_received_email"
		, CASE WHEN last_ever_sent_emails_timestamp is not NULL THEN 1 ELSE 0 END AS "did_sent_email_ever"
		, floor(EXTRACT(EPOCH FROM (last_ever_sent_emails_timestamp - join_ts)) / 86400) AS "days_to_sent_email"
		, CASE WHEN last_ever_sales_meetings_timestamp is not NULL THEN 1 ELSE 0 END AS "did_sales_meeting_ever"
		, floor(EXTRACT(EPOCH FROM (last_ever_sales_meetings_timestamp - join_ts)) / 86400) AS "days_to_sales_meeting"
		, DATE_TRUNC('month', timestamp) AS "month"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_transactions_timestamp)) / 86400) AS "days_since_last_transaction"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_clicked_docsends_timestamp)) / 86400) AS "days_since_last_docsend"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_opened_emails_timestamp)) / 86400) AS "days_since_last_email_open"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_received_emails_timestamp)) / 86400) AS "days_since_last_received_email"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_sent_emails_timestamp)) / 86400) AS "days_since_last_sent_email"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_voicemails_timestamp)) / 86400) AS "days_since_last_voicemail"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_calls_timestamp)) / 86400) AS "days_since_last_call"
		, floor(EXTRACT(EPOCH FROM (NOW() - timestamp)) / 86400) AS "days_since_added_to_crm"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_marketplace_signups_timestamp)) / 86400) AS "days_since_mp_signup"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_deal_createds_timestamp)) / 86400) AS "days_since_last_product_deal_created"
		, floor(EXTRACT(EPOCH FROM (NOW() - last_ever_sales_meetings_timestamp)) / 86400) AS "days_since_last_meeting"
		, floor(EXTRACT(EPOCH FROM (NOW() - first_ever_lead_funnel_entereds_timestamp)) / 86400) AS "days_since_lead_funnel_entered"
	FROM (
 		SELECT
			s.activity_id
			, s.ts AS "timestamp"
	...
	LEFT JOIN (
 		SELECT
			COALESCE(s.customer, s.anonymous_customer_id) AS "join_customer"
			, COUNT(CASE WHEN ( s.activity = 'call' AND s.feature_json ->> 'a2' = 'Connected' ) THEN s.ts END) AS "total_calls_ever"
			, COUNT(CASE WHEN s.activity = 'voicemail' THEN s.ts END) AS "total_voicemails_ever"
			, MAX(CASE WHEN ( s.activity = 'call' AND s.feature_json ->> 'a2' = 'Connected' ) THEN s.ts END) AS "last_ever_calls_timestamp"
			, MAX(CASE WHEN s.activity = 'voicemail' THEN s.ts END) AS "last_ever_voicemails_timestamp"
			, COUNT(CASE WHEN ( s.activity = 'opened_email' AND s.feature_json ->> 'a3' ilike '%mailbox%' ) THEN s.ts END) AS "total_opened_emails_ever"
			, MAX(CASE WHEN ( s.activity = 'opened_email' AND s.feature_json ->> 'a3' ilike '%mailbox%' ) THEN s.ts END) AS "last_ever_opened_emails_timestamp"
			...
			, COUNT(CASE WHEN ( s.activity = 'transaction' AND CAST(s.feature_json ->> 'a2' AS FLOAT) > 0.0 ) THEN s.ts END) AS "total_transactions_ever"
			, SUM(CASE WHEN ( s.activity = 'transaction' AND CAST(s.feature_json ->> 'a2' AS FLOAT) > 0.0 ) THEN CAST(s.feature_json ->> 'a2' AS FLOAT) END) AS "sum_of_transaction_volume_usd_ever"
			, MAX(CASE WHEN ( s.activity = 'transaction' AND CAST(s.feature_json ->> 'a2' AS FLOAT) > 0.0 ) THEN s.ts END) AS "last_ever_transactions_timestamp"
			, MAX(CASE WHEN s.activity = 'marketplace_signup' THEN s.ts END) AS "last_ever_marketplace_signups_timestamp"
			, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'marketplace_signup' THEN CONCAT(DATE_TRUNC('second', s.ts), COALESCE(s.feature_json ->> 'a1','')) END) ,20, 1000),'') AS "last_ever_how_did_you_hear_about_us"
			...
			, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'deal_created' THEN CONCAT(DATE_TRUNC('second', s.ts), COALESCE(s.feature_json ->> 'a3','')) END) ,20, 1000),'') AS "last_ever_deal_channel"
			, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'deal_created' THEN CONCAT(DATE_TRUNC('second', s.ts), COALESCE(s.link,'')) END) ,20, 1000),'') AS "last_ever_deal_createds_link"
			...
			, COUNT(CASE WHEN ( s.activity = 'received_email' AND s.feature_json ->> 'a3' ilike '%mailbox%' ) THEN s.ts END) AS "total_received_emails_ever"
			, MAX(CASE WHEN ( s.activity = 'received_email' AND s.feature_json ->> 'a3' ilike '%mailbox%' ) THEN s.ts END) AS "last_ever_received_emails_timestamp"
