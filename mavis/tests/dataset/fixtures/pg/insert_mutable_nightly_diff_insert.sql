 ...
 (SELECT
	...
	, json_build_object('a1', s.feature_1, 'a2', s.feature_2, 'a3', s.feature_3) AS "feature_json"
FROM (
	SELECT
...
WHERE (
	CAST(s.ts AS DATE) > CAST('2021-11-03' AS DATE)  AND
	s.ts <= CAST('2022-05-18T23:19:49.117237' AS TIMESTAMP)  AND
...
WHERE (
	...
	CAST(d.ts AS DATE) > CAST('2021-11-03' AS DATE)  AND
	d.ts <= CAST('2022-05-18T23:19:49.117237' AS TIMESTAMP)
))
))
