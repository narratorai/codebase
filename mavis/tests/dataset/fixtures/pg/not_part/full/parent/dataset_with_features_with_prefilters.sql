...
LEFT JOIN (
 	SELECT
		...
		, NULLIF(SUBSTRING(MIN(CASE WHEN ( s.activity = 'company_started_subscription' AND s.link is not NULL ) THEN CONCAT(DATE_TRUNC('second', s.ts), COALESCE(s.link,'')) END) ,20, 1000),'') AS "first_ever_company_started_subscriptions_link"
	FROM (
 		SELECT
			*
		FROM test_schema.activity_stream AS s
...
LEFT JOIN (
	...
	FROM (
		SELECT
			...
			, c.join_cohort_id
			, s.ts
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(floor(EXTRACT(EPOCH FROM (s.ts - c.join_ts)) / 60)) < 30 ) THEN s.ts END AS "first_in_between_viewed_dashboards_timestamp"
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(floor(EXTRACT(EPOCH FROM (s.ts - c.join_ts)) / 60)) < 30 ) THEN s.feature_json ->> 'company_slug' END AS "first_in_between_company_slug"
			, CASE WHEN ( s.activity = 'viewed_dashboard' AND ABS(floor(EXTRACT(EPOCH FROM (s.ts - c.join_ts)) / 60)) < 30 ) THEN s.feature_json ->> 'slug' END AS "first_in_between_slug"
		FROM (
 			SELECT
				s.activity_id
				, s.ts AS "timestamp"
		...
		INNER JOIN (
			...
			ON (
				COALESCE(s.customer, s.anonymous_customer_id) = c.join_customer  AND
				ABS(floor(EXTRACT(EPOCH FROM (s.ts - c.join_ts)) / 60)) < 30  AND
				s.ts > c.join_ts  AND
