...
cohort AS (
	SELECT
		...
		, JSON_VALUE(s.feature_json['a1']) AS product_identifier
		, JSON_VALUE(s.feature_json['a2']) AS is_trial_conversion
		, JSON_VALUE(s.feature_json['a3']) AS renewal_number
		, s.revenue_impact
...
SELECT
	...
	, FLOOR(TIMESTAMP_DIFF(first_after_purchased_premium_timestamp, join_ts, second)/86400) AS days_to_purchased_premium
FROM cohort AS c
