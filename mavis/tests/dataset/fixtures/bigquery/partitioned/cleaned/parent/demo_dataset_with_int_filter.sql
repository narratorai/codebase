...
cohort_stream AS (
	SELECT
		*
	FROM test_schema.activity_stream_added_to_crm AS s
),
ever_stream AS (
	SELECT
		*
	FROM test_schema.activity_stream_voicemail AS s
	UNION ALL
		SELECT
		*
	FROM test_schema.activity_stream_transaction AS s
	UNION ALL
		SELECT
		*
	FROM test_schema.activity_stream_sent_email AS s
	UNION ALL
		SELECT
		*
	FROM test_schema.activity_stream_sales_meeting AS s
	UNION ALL
		SELECT
		*
	...
	UNION ALL
		SELECT
		*
	FROM test_schema.activity_stream_newsletter_subscription AS s
	UNION ALL
		SELECT
		*
	FROM test_schema.activity_stream_marketplace_signup AS s
	UNION ALL
		SELECT
		*
	FROM test_schema.activity_stream_lead_funnel_entered AS s
	UNION ALL
		SELECT
		*
	FROM test_schema.activity_stream_deal_created AS s
	UNION ALL
		SELECT
		*
	FROM test_schema.activity_stream_click_docsend AS s
	UNION ALL
		SELECT
		*
	FROM test_schema.activity_stream_call AS s
),
cohort AS (
	SELECT
		s.activity_id
...
append_ever AS (
	SELECT
		COALESCE(s.customer, s.anonymous_customer_id) AS join_customer
		, COUNT(CASE WHEN ( s.activity = "call" AND JSON_VALUE(s.feature_json['a2']) = "Connected" ) THEN s.ts END) AS total_calls_ever
		, COUNT(CASE WHEN s.activity = "voicemail" THEN s.ts END) AS total_voicemails_ever
		, MAX(CASE WHEN ( s.activity = "call" AND JSON_VALUE(s.feature_json['a2']) = "Connected" ) THEN s.ts END) AS last_ever_calls_timestamp
		, MAX(CASE WHEN s.activity = "voicemail" THEN s.ts END) AS last_ever_voicemails_timestamp
		, COUNT(CASE WHEN ( s.activity = "opened_email" AND lower(JSON_VALUE(s.feature_json['a3'])) like "%mailbox%" ) THEN s.ts END) AS total_opened_emails_ever
		, MAX(CASE WHEN ( s.activity = "opened_email" AND lower(JSON_VALUE(s.feature_json['a3'])) like "%mailbox%" ) THEN s.ts END) AS last_ever_opened_emails_timestamp
		...
		, COUNT(CASE WHEN ( s.activity = "transaction" AND SAFE_CAST( JSON_VALUE(s.feature_json['a2']) AS FLOAT64) > 0.0 ) THEN s.ts END) AS total_transactions_ever
		, SUM(CASE WHEN ( s.activity = "transaction" AND SAFE_CAST( JSON_VALUE(s.feature_json['a2']) AS FLOAT64) > 0.0 ) THEN SAFE_CAST( JSON_VALUE(s.feature_json['a2']) AS FLOAT64) END) AS sum_of_transaction_volume_usd_ever
		, MAX(CASE WHEN ( s.activity = "transaction" AND SAFE_CAST( JSON_VALUE(s.feature_json['a2']) AS FLOAT64) > 0.0 ) THEN s.ts END) AS last_ever_transactions_timestamp
		, MAX(CASE WHEN s.activity = "marketplace_signup" THEN s.ts END) AS last_ever_marketplace_signups_timestamp
		, NULLIF(SUBSTR(MAX(CASE WHEN s.activity = "marketplace_signup" THEN CONCAT(CAST(TIMESTAMP_TRUNC(s.ts, SECOND) AS string), coalesce(JSON_VALUE(s.feature_json['a1']),'')) END), 23, 1000), '') AS last_ever_how_did_you_hear_about_us
		...
		, NULLIF(SUBSTR(MAX(CASE WHEN s.activity = "deal_created" THEN CONCAT(CAST(TIMESTAMP_TRUNC(s.ts, SECOND) AS string), coalesce(JSON_VALUE(s.feature_json['a3']),'')) END), 23, 1000), '') AS last_ever_deal_channel
		, NULLIF(SUBSTR(MAX(CASE WHEN s.activity = "deal_created" THEN CONCAT(CAST(TIMESTAMP_TRUNC(s.ts, SECOND) AS string), coalesce(s.link,'')) END), 23, 1000), '') AS last_ever_deal_createds_link
		...
		, COUNT(CASE WHEN ( s.activity = "received_email" AND lower(JSON_VALUE(s.feature_json['a3'])) like "%mailbox%" ) THEN s.ts END) AS total_received_emails_ever
		, MAX(CASE WHEN ( s.activity = "received_email" AND lower(JSON_VALUE(s.feature_json['a3'])) like "%mailbox%" ) THEN s.ts END) AS last_ever_received_emails_timestamp
...
FROM (
	SELECT
		...
		, COALESCE(ever.total_calls_ever, 0) AS total_calls_ever
		, COALESCE(ever.total_voicemails_ever, 0) AS total_voicemails_ever
		, ever.last_ever_calls_timestamp
		, ever.last_ever_voicemails_timestamp
		, COALESCE(ever.total_opened_emails_ever, 0) AS total_opened_emails_ever
		, ever.last_ever_opened_emails_timestamp
		, COALESCE(ever.total_clicked_docsends_ever, 0) AS total_clicked_docsends_ever
		, ever.last_ever_clicked_docsends_timestamp
		, COALESCE(ever.total_transactions_ever, 0) AS total_transactions_ever
		, COALESCE(ever.sum_of_transaction_volume_usd_ever, 0) AS sum_of_transaction_volume_usd_ever
		...
		, COALESCE(ever.total_deal_createds_ever, 0) AS total_deal_createds_ever
		...
		, COALESCE(ever.total_received_emails_ever, 0) AS total_received_emails_ever
		, ever.last_ever_received_emails_timestamp
		, COALESCE(ever.total_sent_emails_ever, 0) AS total_sent_emails_ever
		, ever.last_ever_sent_emails_timestamp
		, COALESCE(ever.total_sales_meetings_ever, 0) AS total_sales_meetings_ever
		...
		, FLOOR(TIMESTAMP_DIFF(last_ever_calls_timestamp, join_ts, second)/86400) AS days_to_call
		, CASE WHEN last_ever_voicemails_timestamp is not NULL THEN 1 ELSE 0 END AS did_voicemail_ever
		, FLOOR(TIMESTAMP_DIFF(last_ever_voicemails_timestamp, join_ts, second)/86400) AS days_to_voicemail
		, CASE WHEN last_ever_opened_emails_timestamp is not NULL THEN 1 ELSE 0 END AS did_opened_email_ever
		, FLOOR(TIMESTAMP_DIFF(last_ever_opened_emails_timestamp, join_ts, second)/86400) AS days_to_opened_email
		, CASE WHEN last_ever_clicked_docsends_timestamp is not NULL THEN 1 ELSE 0 END AS did_clicked_docsend_ever
		, FLOOR(TIMESTAMP_DIFF(last_ever_clicked_docsends_timestamp, join_ts, second)/86400) AS days_to_clicked_docsend
		, CASE WHEN last_ever_transactions_timestamp is not NULL THEN 1 ELSE 0 END AS did_transaction_ever
		, FLOOR(TIMESTAMP_DIFF(last_ever_transactions_timestamp, join_ts, second)/86400) AS days_to_transaction
		, CASE WHEN last_ever_marketplace_signups_timestamp is not NULL THEN 1 ELSE 0 END AS did_marketplace_signup_ever
		, FLOOR(TIMESTAMP_DIFF(last_ever_marketplace_signups_timestamp, join_ts, second)/86400) AS days_to_marketplace_signup
		, CASE WHEN last_ever_deal_createds_timestamp is not NULL THEN 1 ELSE 0 END AS did_deal_created_ever
		, FLOOR(TIMESTAMP_DIFF(last_ever_deal_createds_timestamp, join_ts, second)/86400) AS days_to_deal_created
		, CASE WHEN first_ever_lead_funnel_entereds_timestamp is not NULL THEN 1 ELSE 0 END AS did_lead_funnel_entered_ever
		, FLOOR(TIMESTAMP_DIFF(join_ts, first_ever_lead_funnel_entereds_timestamp, second)/86400) AS days_from_lead_funnel_entered
		...
		, FLOOR(TIMESTAMP_DIFF(join_ts, first_ever_subscribed_to_newsletters_timestamp, second)/86400) AS days_from_subscribed_to_newsletter
		, CASE WHEN last_ever_received_emails_timestamp is not NULL THEN 1 ELSE 0 END AS did_received_email_ever
		, FLOOR(TIMESTAMP_DIFF(last_ever_received_emails_timestamp, join_ts, second)/86400) AS days_to_received_email
		, CASE WHEN last_ever_sent_emails_timestamp is not NULL THEN 1 ELSE 0 END AS did_sent_email_ever
		, FLOOR(TIMESTAMP_DIFF(last_ever_sent_emails_timestamp, join_ts, second)/86400) AS days_to_sent_email
		, CASE WHEN last_ever_sales_meetings_timestamp is not NULL THEN 1 ELSE 0 END AS did_sales_meeting_ever
		, FLOOR(TIMESTAMP_DIFF(last_ever_sales_meetings_timestamp, join_ts, second)/86400) AS days_to_sales_meeting
		, TIMESTAMP_TRUNC(timestamp, month) AS month
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_transactions_timestamp, second)/86400) AS days_since_last_transaction
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_clicked_docsends_timestamp, second)/86400) AS days_since_last_docsend
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_opened_emails_timestamp, second)/86400) AS days_since_last_email_open
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_received_emails_timestamp, second)/86400) AS days_since_last_received_email
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_sent_emails_timestamp, second)/86400) AS days_since_last_sent_email
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_voicemails_timestamp, second)/86400) AS days_since_last_voicemail
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_calls_timestamp, second)/86400) AS days_since_last_call
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), timestamp, second)/86400) AS days_since_added_to_crm
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_marketplace_signups_timestamp, second)/86400) AS days_since_mp_signup
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_deal_createds_timestamp, second)/86400) AS days_since_last_product_deal_created
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_ever_sales_meetings_timestamp, second)/86400) AS days_since_last_meeting
		, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), first_ever_lead_funnel_entereds_timestamp, second)/86400) AS days_since_lead_funnel_entered
	FROM cohort AS c
