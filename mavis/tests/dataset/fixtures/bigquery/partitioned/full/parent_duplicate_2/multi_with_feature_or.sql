...
append_ever AS (
	SELECT
		...
		, NULLIF(SUBSTR(MAX(CONCAT(CAST(TIMESTAMP_TRUNC(s.last_ever_submitted_nps_score_timestamp, SECOND) AS string), coalesce(nps_score_submitted_by,''))), 23, 1000), '') AS nps_score_submitted_by
		...
		, NULLIF(SUBSTR(MAX(CONCAT(CAST(TIMESTAMP_TRUNC(s.last_deal_closed_at, SECOND) AS string), coalesce(sales_person,''))), 23, 1000), '') AS sales_person
	FROM (
		SELECT
			s.customer AS join_customer
			, s.ts
			, CASE WHEN ( s.activity in ("created_manufacturing_run", "created_sales_order", "created_purchase_order", "created_metric_transaction") AND s.ts >= TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL -7 day) ) THEN s.ts END AS total_transactions_in_last_7_days
			, CASE WHEN ( s.activity in ("created_manufacturing_run", "created_sales_order", "created_purchase_order", "created_metric_transaction") AND s.ts >= TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL -7 day) ) THEN s.ts END AS last_used_product_at
			...
			, CASE WHEN ( s.activity = "submitted_nps_score" AND CASE WHEN JSON_VALUE(s.feature_json['a3']) in (NULL, '') THEN NULL ELSE (LOWER(JSON_VALUE(s.feature_json['a3'])) = "true") END = True AND ROW_NUMBER() over (PARTITION by s.activity, s.customer, CASE WHEN CASE WHEN JSON_VALUE(s.feature_json['a3']) in (NULL, '') THEN NULL ELSE (LOWER(JSON_VALUE(s.feature_json['a3'])) = "true") END = True THEN 1 END ORDER BY s.ts desc) = 1 ) THEN SAFE_CAST( JSON_VALUE(s.feature_json['a1']) AS FLOAT64) END AS last_nps_score
...
raw_dataset AS (
	...
	FROM (
		SELECT
			...
			, ever.sales_person
			, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), first_ever_closed_won_deal_timestamp, second)/2592000) AS tenure_in_months
			, FLOOR(TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), last_used_product_at, second)/86400) AS days_since_last_transaction
			, TIMESTAMP_TRUNC(last_ever_submitted_nps_score_timestamp, day) AS last_nps_date
		FROM cohort AS c
		LEFT JOIN append_ever AS ever
			ON c.join_customer = ever.join_customer
	)
	WHERE (
		total_transactions_in_last_7_days = 0.0  AND
		last_nps_score < 7.0
	)
)
SELECT
