...
WHERE activity_occurrence is NULL)
	...
	WHERE (
		...
		COALESCE(activity_repeated_at, SAFE_CAST( "1900-01-01T01:00:00" AS TIMESTAMP)) <> new_activity_repeated_at
	)
) AS s
WHERE
	(
		activity_stream.activity = s.activity  AND
		activity_stream.activity_id = s.activity_id
	)
