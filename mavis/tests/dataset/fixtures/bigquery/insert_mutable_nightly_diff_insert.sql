 ...
 (SELECT
	...
	, s.link
	, SAFE_CAST( s.revenue_impact AS FLOAT64) AS revenue_impact
	...
	, TO_JSON(STRUCT(s.feature_1 as a1, s.feature_2 as a2, s.feature_3 as a3)) AS feature_json
FROM (
	SELECT
...
WHERE (
	SAFE_CAST( s.ts AS TIMESTAMP) > SAFE_CAST( "2021-11-03" AS TIMESTAMP)  AND
	s.ts <= SAFE_CAST( "2022-05-18T23:19:49.117237" AS TIMESTAMP)  AND
	s.activity_id not in (SELECT
	d.activity_id
FROM test_schema.activity_stream AS d
WHERE (
	d.activity = "started_session"  AND
	d._activity_source = "started_sessions"  AND
	SAFE_CAST( d.ts AS TIMESTAMP) > SAFE_CAST( "2021-11-03" AS TIMESTAMP)  AND
	d.ts <= SAFE_CAST( "2022-05-18T23:19:49.117237" AS TIMESTAMP)
))
))
