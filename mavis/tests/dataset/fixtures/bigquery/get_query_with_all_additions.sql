SELECT
	*
	, LEAD(s.ts) over (PARTITION by COALESCE(customer, anonymous_customer_id) ORDER BY s.ts) AS activity_repeated_at
	, ROW_NUMBER() over (PARTITION by COALESCE(customer, anonymous_customer_id) ORDER BY s.ts) AS activity_occurrence
FROM (
	SELECT
		s.activity
		, s.activity_id
