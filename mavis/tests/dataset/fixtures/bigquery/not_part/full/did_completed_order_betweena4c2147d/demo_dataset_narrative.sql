...
cohort_stream AS (
	...
	WHERE (
		...
		SAFE_CAST( s.ts AS TIMESTAMP) > SAFE_CAST( "2021-09-01" AS TIMESTAMP)
	)
),
...
cohort AS (
	...
	WHERE SAFE_CAST( c.timestamp AS TIMESTAMP) > SAFE_CAST( "2021-09-01" AS TIMESTAMP)
),
...
FROM (
	...
	FROM (
		...
		GROUP BY rounded_did_completed_order_between
	)
)
