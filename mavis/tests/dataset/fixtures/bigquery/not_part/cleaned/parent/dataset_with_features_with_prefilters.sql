...
append_ever AS (
	SELECT
		...
		, NULLIF(SUBSTR(MIN(CASE WHEN ( s.activity = "company_started_subscription" AND s.link is not NULL ) THEN CONCAT(CAST(TIMESTAMP_TRUNC(s.ts, SECOND) AS string), COALESCE(s.link,'') ) END), 23, 1000), '') AS first_ever_company_started_subscriptions_link
...
append_in_between AS (
	...
	FROM (
		SELECT
			...
			, c.join_cohort_id
			, s.ts
			, CASE WHEN ( s.activity = "viewed_dashboard" AND ABS(FLOOR(TIMESTAMP_DIFF(s.ts, c.join_ts, second)/60)) < 30 ) THEN s.ts END AS first_in_between_viewed_dashboards_timestamp
			, CASE WHEN ( s.activity = "viewed_dashboard" AND ABS(FLOOR(TIMESTAMP_DIFF(s.ts, c.join_ts, second)/60)) < 30 ) THEN JSON_VALUE(s.feature_json['company_slug']) END AS first_in_between_company_slug
			, CASE WHEN ( s.activity = "viewed_dashboard" AND ABS(FLOOR(TIMESTAMP_DIFF(s.ts, c.join_ts, second)/60)) < 30 ) THEN JSON_VALUE(s.feature_json['slug']) END AS first_in_between_slug
		FROM cohort AS c
		INNER JOIN in_between_stream AS s
			ON (
				COALESCE(s.customer, s.anonymous_customer_id) = c.join_customer  AND
				ABS(FLOOR(TIMESTAMP_DIFF(s.ts, c.join_ts, second)/60)) < 30  AND
				s.ts > c.join_ts  AND
