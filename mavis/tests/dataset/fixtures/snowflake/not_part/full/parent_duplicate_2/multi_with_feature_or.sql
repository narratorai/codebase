...
append_ever AS (
	SELECT
		...
		, NULLIF(SUBSTRING(MAX(CONCAT(LEFT(CAST(s.last_ever_submitted_nps_score_timestamp as string), 19), NVL(nps_score_submitted_by,''))),20, 1000),'') AS nps_score_submitted_by
		...
		, NULLIF(SUBSTRING(MAX(CONCAT(LEFT(CAST(s.last_deal_closed_at as string), 19), NVL(sales_person,''))),20, 1000),'') AS sales_person
	FROM (
		SELECT
			s.customer AS join_customer
			, s.ts
			, CASE WHEN ( s.activity in ('created_manufacturing_run', 'created_sales_order', 'created_purchase_order', 'created_metric_transaction') AND s.ts >= DATEADD(day, -7, SYSDATE()) ) THEN s.ts END AS total_transactions_in_last_7_days
			, CASE WHEN ( s.activity in ('created_manufacturing_run', 'created_sales_order', 'created_purchase_order', 'created_metric_transaction') AND s.ts >= DATEADD(day, -7, SYSDATE()) ) THEN s.ts END AS last_used_product_at
...
raw_dataset AS (
	...
	FROM (
		SELECT
			...
			, ever.sales_person
			, FLOOR(DATEDIFF(second, first_ever_closed_won_deal_timestamp, SYSDATE())/2592000) AS tenure_in_months
			, FLOOR(DATEDIFF(second, last_used_product_at, SYSDATE())/86400) AS days_since_last_transaction
