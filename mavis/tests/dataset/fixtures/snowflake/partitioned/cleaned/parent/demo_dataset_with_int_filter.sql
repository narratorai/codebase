...
append_ever AS (
	SELECT
		...
		, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'marketplace_signup' THEN CONCAT(LEFT(CAST(s.ts as string), 19), NVL(CAST(s.feature_json['a1'] AS VARCHAR(4096)),'')) END ),20, 1000),'') AS last_ever_how_did_you_hear_about_us
		...
		, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'deal_created' THEN CONCAT(LEFT(CAST(s.ts as string), 19), NVL(CAST(s.feature_json['a3'] AS VARCHAR(4096)),'')) END ),20, 1000),'') AS last_ever_deal_channel
		, NULLIF(SUBSTRING(MAX(CASE WHEN s.activity = 'deal_created' THEN CONCAT(LEFT(CAST(s.ts as string), 19), NVL(s.link,'')) END ),20, 1000),'') AS last_ever_deal_createds_link
...
FROM (
	SELECT
		...
		, FLOOR(DATEDIFF(second, last_ever_transactions_timestamp, SYSDATE())/86400) AS days_since_last_transaction
		, FLOOR(DATEDIFF(second, last_ever_clicked_docsends_timestamp, SYSDATE())/86400) AS days_since_last_docsend
		, FLOOR(DATEDIFF(second, last_ever_opened_emails_timestamp, SYSDATE())/86400) AS days_since_last_email_open
		, FLOOR(DATEDIFF(second, last_ever_received_emails_timestamp, SYSDATE())/86400) AS days_since_last_received_email
		, FLOOR(DATEDIFF(second, last_ever_sent_emails_timestamp, SYSDATE())/86400) AS days_since_last_sent_email
		, FLOOR(DATEDIFF(second, last_ever_voicemails_timestamp, SYSDATE())/86400) AS days_since_last_voicemail
		, FLOOR(DATEDIFF(second, last_ever_calls_timestamp, SYSDATE())/86400) AS days_since_last_call
		, FLOOR(DATEDIFF(second, timestamp, SYSDATE())/86400) AS days_since_added_to_crm
		, FLOOR(DATEDIFF(second, last_ever_marketplace_signups_timestamp, SYSDATE())/86400) AS days_since_mp_signup
		, FLOOR(DATEDIFF(second, last_ever_deal_createds_timestamp, SYSDATE())/86400) AS days_since_last_product_deal_created
		, FLOOR(DATEDIFF(second, last_ever_sales_meetings_timestamp, SYSDATE())/86400) AS days_since_last_meeting
		, FLOOR(DATEDIFF(second, first_ever_lead_funnel_entereds_timestamp, SYSDATE())/86400) AS days_since_lead_funnel_entered
	FROM cohort AS c
