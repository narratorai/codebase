...
append_before AS (
	...
	FROM (
		SELECT
			...
			, NULLIF(SUBSTRING(MAX(CONCAT(LEFT(CAST(s.last_before_email_link_clicked_timestamp as string), 19), NVL(last_before_email_link_clicked_link,''))),20, 1000),'') AS last_before_email_link_clicked_link
			, NULLIF(SUBSTRING(MAX(CONCAT(LEFT(CAST(s.last_before_email_link_clicked_timestamp as string), 19), NVL(last_before_campaign_name,''))),20, 1000),'') AS last_before_campaign_name
			, MAX(last_before_payment_transactions_timestamp) AS last_before_payment_transactions_timestamp
			, NULLIF(SUBSTRING(MAX(CONCAT(LEFT(CAST(s.last_before_payment_transactions_timestamp as string), 19), NVL(last_before_pro_type,''))),20, 1000),'') AS last_before_pro_type
		...
		FROM (
			SELECT
				...
				, CASE WHEN ( s.activity = 'clicked_email_link' AND ( CAST(s.feature_json['a2'] AS VARCHAR(4096)) not ilike '%password reset%' OR CAST(s.feature_json['a2'] AS VARCHAR(4096)) not ilike '%validation%' ) AND ABS(FLOOR(DATEDIFF(second, s.ts, c.join_ts)/86400)) < 30 ) THEN s.link END AS last_before_email_link_clicked_link
				, CASE WHEN ( s.activity = 'clicked_email_link' AND ( CAST(s.feature_json['a2'] AS VARCHAR(4096)) not ilike '%password reset%' OR CAST(s.feature_json['a2'] AS VARCHAR(4096)) not ilike '%validation%' ) AND ABS(FLOOR(DATEDIFF(second, s.ts, c.join_ts)/86400)) < 30 ) THEN CAST(s.feature_json['a2'] AS VARCHAR(4096)) END AS last_before_campaign_name
