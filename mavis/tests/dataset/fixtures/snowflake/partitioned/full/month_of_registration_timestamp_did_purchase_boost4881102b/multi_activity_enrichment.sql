...
append_after AS (
	...
	FROM (
		SELECT
			...
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'purchased_premium' THEN CONCAT(LEFT(CAST(s.ts as string), 19), NVL(CAST(s.feature_json['a2'] AS VARCHAR(4096)),'')) END ),20, 1000),'') AS first_purchase_is_trial_conversion
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'purchased_premium' THEN CONCAT(LEFT(CAST(s.ts as string), 19), NVL(s.activity_id,'')) END ),20, 1000),'') AS join_enriched_activity_id_1_a
			, MIN(CASE WHEN s.activity = 'bought_ppp' THEN s.ts END) AS first_ppp_bought_timestamp
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'bought_ppp' THEN CONCAT(LEFT(CAST(s.ts as string), 19), NVL(CAST(s.feature_json['a1'] AS VARCHAR(4096)),'')) END ),20, 1000),'') AS first_ppp_feature
			, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'bought_ppp' THEN CONCAT(LEFT(CAST(s.ts as string), 19), NVL(CAST(s.feature_json['a2'] AS VARCHAR(4096)),'')) END ),20, 1000),'') AS first_ppp_location
			, COUNT(CASE WHEN ( s.activity in ('purchased_premium', 'made_purchase') AND ABS(FLOOR(DATEDIFF(second, c.join_ts, s.ts)/86400)) < 100 AND ( enriched_purchase_tbl.is_change_during_sub_period <> True OR enriched_purchase_tbl.is_change_during_sub_period is NULL ) ) THEN s.ts END) AS total_purchases
			, SUM(CASE WHEN ( s.activity in ('purchased_premium', 'made_purchase') AND ABS(FLOOR(DATEDIFF(second, c.join_ts, s.ts)/86400)) < 100 AND ( enriched_purchase_tbl.is_change_during_sub_period <> True OR enriched_purchase_tbl.is_change_during_sub_period is NULL ) ) THEN s.revenue_impact END) AS a_3_mo_ltv
...
raw_dataset AS (
	...
	FROM (
		SELECT
			...
			, FLOOR(DATEDIFF(second, join_ts, first_trial_timestamp)/86400) AS days_to_first_redeem_trial
			, CASE WHEN first_premium_purchase_timestamp is not NULL THEN 1 ELSE 0 END AS did_purchased_premium
			, FLOOR(DATEDIFF(second, join_ts, first_premium_purchase_timestamp)/86400) AS days_to_first_purchased_premium
			, CASE WHEN first_ppp_bought_timestamp is not NULL THEN 1 ELSE 0 END AS did_trigger_bought_ppp
			, FLOOR(DATEDIFF(second, join_ts, first_ppp_bought_timestamp)/86400) AS days_to_first_bought_ppp
			, CASE WHEN first_boost_purchase_timestamp is not NULL THEN 1 ELSE 0 END AS did_purchase_boost
			, FLOOR(DATEDIFF(second, join_ts, first_boost_purchase_timestamp)/86400) AS days_to_first_boost_purchase
...
SELECT
	*
	, RATIO_TO_REPORT(total_customers) OVER (PARTITION BY month_of_registration_timestamp) AS conversion_rate_to_purchase_boost
FROM (
	SELECT
		...
		, AVG(1.0000*rd.did_redeem_trial) AS conversion_rate_to_redeem_trial
		, AVG(1.0000*rd.days_to_first_redeem_trial) AS average_days_to_first_redeem_trial
		, SUM(rd.did_purchased_premium) AS total_purchased_premium
		, AVG(1.0000*rd.did_purchased_premium) AS conversion_rate_to_purchased_premium
		, AVG(1.0000*rd.days_to_first_purchased_premium) AS average_days_to_first_purchased_premium
		, SUM(rd.did_trigger_bought_ppp) AS total_trigger_bought_ppp
		, AVG(1.0000*rd.did_trigger_bought_ppp) AS conversion_rate_to_trigger_bought_ppp
		, AVG(1.0000*rd.days_to_first_bought_ppp) AS average_days_to_first_bought_ppp
		, SUM(rd.total_purchases) AS total_purchases
		, AVG(1.0000*rd.total_purchases) AS average_total_purchases
		, SUM(rd.a_3_mo_ltv) AS a_3_mo_ltv
		, AVG(1.0000*rd.a_3_mo_ltv) AS average_3_mo_ltv
		, AVG(1.0000*rd.days_to_first_boost_purchase) AS average_days_to_first_boost_purchase
	FROM raw_dataset AS rd
	GROUP BY rd.month_of_registration_timestamp, rd.did_purchase_boost
)
