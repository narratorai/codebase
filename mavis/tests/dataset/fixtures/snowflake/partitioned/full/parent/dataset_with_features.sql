...
append_ever AS (
	SELECT
		...
		, NULLIF(SUBSTRING(MIN(CASE WHEN s.activity = 'company_started_subscription' THEN CONCAT(LEFT(CAST(s.ts as string), 19), NVL(s.link,'')) END ),20, 1000),'') AS first_ever_company_started_subscriptions_link
