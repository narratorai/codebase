...
module "irsa" {
  source                            = "../irsa"
  create_kubernetes_namespace       = try(local.irsa_config.create_kubernetes_namespace, false)
  create_kubernetes_service_account = try(local.irsa_config.create_kubernetes_service_account, true)
  kubernetes_namespace              = lookup(local.irsa_config, "kubernetes_namespace", false)
