...
output "irsa_arn" {
  ...
  value       = try(module.irsa.irsa_iam_role_arn, null)
}

output "irsa_name" {
  description = "IAM role name for the service account"
  value       = try(module.irsa.irsa_iam_role_name, null)
}

output "service_account" {
  description = "Name of Kubernetes service account"
  value       = try(coalesce(try(module.irsa.service_account, null), lookup(var.irsa_config, "kubernetes_service_account", null)), null)
}